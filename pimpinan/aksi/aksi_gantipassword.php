...
if($old_pass != $data['password']){
    echo "<script type='text/javascript'>alert('Password Lama Anda Salah');document.location='../pimpinan_gantipassword.php'</script>";
}
else{
    if($new_pass != $retype_pass){
        echo "<script type='text/javascript'>alert('Password Baru yang Anda Masukkan Tidak Sama');document.location='../pimpinan_gantipassword.php'</script>";
    }
    else{
        $query = mysqli_query($db, "UPDATE tb_akun SET password = '$new_pass' WHERE username = '$username'");
    if($query){
        echo "<script>alert('Password Berhasil Diubah');document.location='../pimpinan_dataakun.php'</script>";
    }
    }
}
